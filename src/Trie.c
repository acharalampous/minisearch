...
int TN_NewPost(TrieNode* leafNode, int id){
    ...
    if(result != NULL) // if it exists in posting list
        PLN_increaseTimes(result); // increas number of occurences
    else
        PL_Push(leafNode->ps,PLN_Init(id)); // create new entry
...
int Trie_Insert(Trie* trie, char* newWord, int id){
